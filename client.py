...
class ClientManager:
    ...
    def update(self, client_id, dataset_ref, model_ref):
        dataset = dataset_ref['dataset'] #ray.get(dataset_ref['dataset'])
        data_split = dataset_ref['split'] #ray.get(dataset_ref['split'])
        label_split = dataset_ref['label_split'] #ray.get(dataset_ref['label_split'])
        local_parameters = model_ref['local_params'] #ray.get(model_ref['local_params'])
